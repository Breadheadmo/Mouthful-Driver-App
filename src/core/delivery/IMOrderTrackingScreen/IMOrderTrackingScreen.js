...
import { Text, View, Dimensions, Image, ScrollView, Platform } from 'react-native'
...
function IMOrderTrackingScreen({ navigation, route }) {
  ...
  if (eta > 0) deliveryDate.setSeconds(deliveryDate.getSeconds() + eta)
  ...
  const latestArrivalString =
    eta > 0
      ? `${latestArrivalDate.getHours().toString().padStart(2, '0')}:${latestArrivalDate
          .getMinutes()
          .toString()
          .padStart(2, '0')}`
      : ''
