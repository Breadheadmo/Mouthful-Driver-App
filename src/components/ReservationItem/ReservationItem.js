...
function ReservationItem(props) {
  const reservations = props.constructorObject
  ...
  const dateComponent = reservation => {
    if (!reservation?.createdAt) {
      return null
    }
    const { month, day, year } = getDate(new Date(reservation.createdAt))
    return (
